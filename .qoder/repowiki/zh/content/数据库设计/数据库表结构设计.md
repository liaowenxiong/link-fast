...
- 字段设计
  ...
  - 流量与状态：flow_total、flow_balance、status、renew
  - 关联与扩展：order_id、order_no、app_order_no、product_no、extend_ip、project_id
- 业务规则
  - instance_no 非空，作为实例唯一标识；status 与 renew 控制实例可用性与续费策略
- DAO 职责
  - 批量更新、按条件分页查询、按 instance_no 更新备注
...
- DAO 层职责
  ...
  - ProxyInstanceDAO：实例批量更新、条件查询、分页统计、备注更新
  - UserDao：用户 CRUD
- 耦合与内聚
